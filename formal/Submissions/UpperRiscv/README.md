...
- **Machine image.** `Program.lean` is a 1337-instruction RV64IM image with a 72-byte data image.
  ...
  level tag, hash in place. The slots, 24 bytes apart, then form the root input. The root length
  is one `ADDI` from the checked signature length still in `x13`, and the decision branches on
  each mismatching word to a rejection placed after the accepting HALT.
