...
/-- Every run, accepting or rejecting, executes at most 696 cycles: one per executed
instruction and twelve for the 6080-bit root hash, with the chain steps charged by the path
taken. -/
theorem submission_cycles : submission.CyclesAtMost 696 := by
  intro pk m bits b cycles completed
  exact (RiscvUpperProgram.image_refines pk m bits).2 b cycles completed

/-- Every requirement of a scored RISC-V submission, at 696 cycles. -/
theorem machineCertificate : submission.Certificate 696 :=
