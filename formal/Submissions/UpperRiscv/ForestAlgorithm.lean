...
`2⁻¹²⁸`, 127-bit strong security, and verification within 170 compressions on every path.
-/
...
theorem cost : scheme.VerifyCostAtMost 170 := by
  apply AlgorithmAdapter.verifyCost Forest.forestScheme (v := 169) (by decide)
  intro i
  have h := Forest.forestScheme_verifyCost i
  change 1 + Forest.forestScheme.graph.reconstructCost (Forest.forestScheme.sets i) = 170 at h
  omega
...
/-- A complete admissible, strongly secure, 170-compression construction. -/
theorem certificate :
    scheme.Admissible (1 / 2 ^ 128) ∧
    scheme.Secure ∧ scheme.VerifyCostAtMost 170 :=
